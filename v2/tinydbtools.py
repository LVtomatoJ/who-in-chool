...
def add_user(email: str, password: str, openid: str, level: int, maxbindnum: int, maxworknum: int) -> int:
    ...
    Returns:
        int: doc_id
    """
    doc_id = user_db.insert({'email': email, 'password': password, 'openid': openid,
                            'level': level, 'maxbindnum': maxbindnum, 'maxworknum': maxworknum})
    return doc_id


def add_work(email: str, bindid: str, templateid: str, starttime: str, endtime: str, workid: str) -> int:
    doc_id = work_db.insert({'email': email, 'bindid': bindid, 'templateid': templateid,
                            'status': 1, 'starttime': starttime, 'endtime': endtime, 'workid': workid})
    return doc_id
...
def add_bind(email: str, bindid: str, password: str, jwsession: str, notes: str, school: str):
    doc_id = bind_db.insert({'email': email, "bindid": bindid, 'password': password,
                            'jwsession': jwsession, 'notes': notes, 'school': school, 'status': 1})
    return doc_id
...
def change_user(email: str, password: str, openid: str, level: int, maxbindnum: int, maxworknum: int):
    users = user_db.update({'password': password, 'openid': openid, 'level': level,
                           'maxbindnum': maxbindnum, 'maxworknum': maxworknum}, Querydb.email == email)
    return users


def change_work(email: str, workid: str, templateid: str, bindid: str, status: int, starttime: str, endtime: str):
    works = work_db.update({'email': email, 'templateid': templateid, 'bindid': bindid,
                           'status': status, 'starttime': starttime, 'endtime': endtime}, Querydb.workid == workid)
    return works


def change_bind(email: str, password: str, jwsession: str, notes: str, school: str, status: int, bindid: str,):
    binds = bind_db.update({'email': email, 'password': password, 'jwsession': jwsession,
                           'notes': notes, 'school': school, "status": status}, Querydb.bindid == bindid)
    return binds


def change_notic(noticid: str, title: str, content: str, time: str, show: int):
    notics = notic_db.update({'title': title, 'content': content,
                             'time': time, 'show': show}, Querydb.noticid == noticid)
    return notics
...
def add_work_log(email: str, bindid: str, workid: str, templateid: str, time: str, code: int, msg: str):
    doc_id = worklog_db.insert({'email': email, 'bindid': bindid, 'workid': workid,
                               'templateid': templateid, 'time': time, 'code': code, 'msg': msg})
    return doc_id
